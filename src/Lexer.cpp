...
std::string Token::get_output_string() {
    return token_output_names[token_output_type] + " " + token_val_string + " Line " + std::to_string(line_nbr);
}
...
char fetch_char() {
    if (cur_ch_idx >= source_file_str.size()) {
        return EOF;
    }
//    assert(cur_ch_idx >= 0 && cur_ch_idx < source_file_str.size());
    cur_ch = source_file_str[cur_ch_idx];
...
Token fetch_token() {
    ...
    if (is_alpha_or_underscore(cur_ch)) { /* identifier or keyword */
        ...
        if (is_keyword(token_buffer)) {
            tokens.push_back(Token(KEYWORD, 0, token_buffer, line_count));
            return tokens.back();
        } else { /* 不是 keyword 就是标识符 */
            tokens.push_back(Token(IDENTIFIER, 0, token_buffer, line_count));
            return tokens.back();
        }
    }
    ...
    if (isdigit(cur_ch)) { /* unsigned int or 0. */
        token_buffer.push_back(cur_ch);
        if (cur_ch == '0') {
            tokens.push_back(Token(ZERO, 0, token_buffer, line_count));
            return tokens.back();
        } else {
            fetch_char();
    ...
    if (cur_ch == EOF) {
        tokens.push_back(Token(END_OF_FILE, -1, "EOF", line_count));
        return tokens.back();
    }
